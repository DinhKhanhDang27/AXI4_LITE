module alu_core #(
    ...
    function automatic logic [DATA_WIDTH-1:0] divide_unsigned(
        input logic [DATA_WIDTH-1:0] numerator,
        input logic [DATA_WIDTH-1:0] denominator
    );
        logic [DATA_WIDTH-1:0] quotient;
        logic [DATA_WIDTH:0]   remainder;

        quotient  = '0;
        remainder = '0;

        for (int i = DATA_WIDTH - 1; i >= 0; i--) begin
            remainder = {remainder[DATA_WIDTH-1:0], numerator[i]};
        ...
        unique case (opcode)
            ...
            OP_DIV: begin
                if (operand_b == '0) begin
                    ...
                    result_next = divide_unsigned(operand_a, operand_b);
                end
            end
            default: begin
                result_next = '0;
